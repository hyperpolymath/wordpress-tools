...
[arabic]
. Navigate to *Constructs → Add New* in WordPress admin
. Enter construct title (e.g., "`Time`")
...
[arabic]
. Go to *Entanglements → Add New*
. Select *Source* construct
. Select *Target* construct
. Set *Relationship Type* (e.g., "`depends on`")
. Set *Strength* (0.0-1.0)
. Check *Bidirectional* if applicable
...
. Edit Rust in `+wasm/semantic_processor/src/+`
. Run `+./build.sh+` to recompile WASM
. Refresh WordPress to see changes
