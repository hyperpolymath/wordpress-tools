...
*Expected*: First 10 requests succeed (200/302), remaining fail with 429
(Rate Limit Exceeded)
...
*Expected*: Security event logs when rate limits trigger or XSS attempts
occur
...
*Solution*: Complete WordPress installation first. MU-plugins activate
after installation.
...
. *Configure Svalinn* with production policy
. *Enable Vörðr* for runtime enforcement
. *Sign with Cerro Torre* for provenance verification
. *Set up monitoring* for security events
. *Configure backups* for WordPress data and database
. *Test all security features* in staging environment

=== Success Criteria

✅ WordPress accessible at http://localhost:8082 ✅ php-aegis MU-plugin
loaded without errors ✅ Security headers present after installation ✅
Rate limiting enforces 10 req/min per IP ✅ XSS payloads sanitized in
post content ✅ XMLRPC blocked ✅ IndieWeb endpoints secure (after
plugin installation) ✅ Cerro Torre can package the container ✅ Svalinn
can enforce verb governance ✅ Vörðr can enforce runtime policies

=== Next Steps

[arabic]
. Complete WordPress installation wizard at http://localhost:8082
. Run verification tests (Tests 1-10)
. Install IndieWeb plugins (Micropub, Webmention)
. Test full security stack with real payloads
. Create Cerro Torre package with cryptographic signature
. Deploy Svalinn gateway for verb governance
. Configure Vörðr for runtime enforcement
