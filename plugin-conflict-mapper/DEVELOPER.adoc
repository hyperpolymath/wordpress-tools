...
* link:#class-structure[Class Structure]
* link:++#hooks--filters++[Hooks & Filters]
* link:#database-schema[Database Schema]
* link:#rest-api-reference[REST API Reference]
* link:#wp-cli-reference[WP-CLI Reference]
...
WP Plugin Conflict Mapper follows a modular, object-oriented
architecture built on WordPress best practices.

==== Design Patterns

* *Singleton*: Main plugin class uses singleton pattern
* *Dependency Injection*: Classes receive dependencies through
constructors
...
*Categories*: SEO, caching, security, backup, forms, ecommerce, social,
analytics, media, email, builder, spam
...
*Metrics*: - File size (MB) - Code complexity (lines + functions +
classes) - Database tables created - Asset files (CSS/JS) - Hook
registrations (actions + filters)

*Rating Scale*: - Excellent: 90-100 - Good: 75-89 - Fair: 60-74 - Poor:
<60
...
*Parameters*: - `+id+` (required): Scan ID
...
*Query Parameters*: - `+limit+` (optional): Number of scans to return
(default: 10)
...
*Options*: - `+--format+`: Output format (table, json, csv). Default:
table - `+--save+`: Save results to database
...
*Arguments*: - `+plugin+`: Plugin slug or name
