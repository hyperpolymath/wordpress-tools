...
All internal services run on isolated `+sinople_internal+` network with
no external access except via nginx reverse proxy.
...
WordPress posts automatically publish to RabbitMQ queue
`+sinople.posts+` in Ecto-compatible format.
...
Already configured in docker-compose. WordPress will use Redis for
object caching.
...
. Upload `+assets/css/min/+` and `+assets/js/dist/+` to CDN
. Update `+functions.php+` with CDN URLs
...
. Verify WASM file exists in `+assets/js/dist/+`
