...
Complete guide to the WP Praxis GraphQL schema design and usage
patterns.

=== Table of Contents

[arabic]
. link:#schema-overview[Schema Overview]
. link:#core-types[Core Types]
. link:#query-patterns[Query Patterns]
. link:#mutation-patterns[Mutation Patterns]
. link:#subscription-patterns[Subscription Patterns]
...
*Key Relationships:* - Can have dependencies on other symbols (by name)
- Can have a rollback symbol for failure recovery - Used by Executions
and Tasks
...
*Key Relationships:* - Contains multiple symbols - Generates executions
for each symbol - Can be referenced by audits
...
*Key Relationships:* - Belongs to a Workflow - Executes a Symbol - May
...
*Key Relationships:* - Referenced by audits - Only one baseline is
typically active at a time
...
*Key Relationships:* - References a Baseline - Optionally references a
Workflow - Contains deviation details
...
*Key Relationships:* - Executes Tasks - Referenced by Executions
...
*Key Relationships:* - Belongs to an Execution - Executes a Symbol - May
be assigned to a Node - Can depend on other Tasks
...
*Cons:* - Performance degrades with large offsets - Inconsistent results
...
*Pros:* - Consistent performance - Stable across data changes

*Cons:* - Can’t jump to specific pages - Slightly more complex to
implement
...
Consider schema versioning for major changes: - Use different endpoints:
`+/graphql/v1+`, `+/graphql/v2+` - Or use field aliases: `+symbolV2+`
