...
Comprehensive GraphQL API for WP Praxis symbolic workflow execution and
introspection.

=== Overview

This GraphQL API provides a complete interface to the WP Praxis symbolic
execution engine, enabling:
...
* *Execution Tracking*: Real-time execution monitoring with detailed
results
...
Visit `+http://localhost:4000/graphql+` in your browser to access the
...
All database queries are batched and cached using DataLoader to prevent
N+1 query problems:
...
`+workflows:delete+`
* `+executions:read+`, `+executions:retry+`, `+executions:cancel+`,
`+executions:rollback+`
* `+baselines:read+`, `+baselines:write+`, `+baselines:activate+`,
`+baselines:delete+`
...
[arabic]
. *Never expose JWT_SECRET*: Use strong, randomly generated secrets
. *Use HTTPS in production*: Encrypt all traffic
. *Validate inputs*: All inputs are validated before execution
. *Limit query complexity*: Prevent resource exhaustion attacks
...
Logs are written to: - Console (development) -
...
*Database Connection Failed* - Verify PostgreSQL is running - Check
database credentials in `+.env+` - Ensure database exists and Ecto
migrations are run

*WebSocket Subscriptions Not Working* - Check that GraphQL endpoint
supports WebSocket protocol - Verify `+graphql-ws+` protocol is used
(not `+subscriptions-transport-ws+`)

*Permission Denied Errors* - Verify JWT token is valid and not expired -
Check user has required permissions/roles

*Slow Queries* - Review DataLoader configuration - Check database
indexes (see Ecto migrations) - Enable query logging to identify
bottlenecks
...
For issues and questions: - Review documentation in `+/Docs+` - Check
example queries in `+client/example-queries.graphql+` - See
`+SCHEMA_GUIDE.md+` for detailed schema documentation
