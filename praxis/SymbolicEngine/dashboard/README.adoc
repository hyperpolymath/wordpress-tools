...
Complete real-time monitoring and control interface for the WP Praxis
symbolic workflow system. Built with Bun, , Elysia, and PostgreSQL.

=== Features

==== Core Dashboard

* *Real-time Statistics*: Live monitoring of workflows, executions,
audits, and system metrics
...
bun install
----
. *Configure database* (edit `+dashboard-config.toml+`):
+
[source,toml]
----
[database]
...
----
. *Configure server* (edit `+dashboard-config.toml+`):
+
[source,toml]
----
[server]
...
The server will start with auto-reload enabled at
`+http://localhost:3000+`
...
*Pagination*: - `+page+`: Page number (default: 1) - `+limit+`: Items
per page (default: 20, max: 100)

*Filtering*: - `+status+`: Filter by status (workflows, executions) -
`+type+`: Filter by type (symbols) - `+workflow_id+`: Filter by workflow
ID
...
. Add styles in `+css/dashboard.css+`
. Add logic in `+js/dashboard.ts+`
