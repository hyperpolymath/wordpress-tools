...
The WP Praxis test suite is a comprehensive, multi-language testing
framework designed to ensure the reliability, performance, and security
of the WP Praxis symbolic system. Tests are organized by layer and
language, reflecting the project’s polyglot architecture.

=== Table of Contents

* link:#quick-start[Quick Start]
* link:#test-structure[Test Structure]
* link:#running-tests[Running Tests]
* link:#writing-tests[Writing Tests]
* link:#coverage-reports[Coverage Reports]
...
. *Use tags* to categorize tests (@Unit, @Integration, @E2E, @Slow,
@Security)
...
Pull requests to `+main+` or `+develop+` - Manual workflow dispatch
...
. *Rust Tests* (Ubuntu)
. *Elixir Tests* (Ubuntu)
. *PHP Tests* (Ubuntu with MySQL)
. ** Tests** (Ubuntu)
. *E2E Tests* (Ubuntu, requires all previous stages)
. *Coverage Report* (Combined coverage)
...
* *`+tests/fixtures/complete-workflow.yaml+`*: Comprehensive workflow
for integration testing
* *`+tests/fixtures/test-symbols.toml+`*: Various symbol definitions for
unit testing
...
. Use appropriate framework for the layer
...
For more information, see: - Project documentation: `+Docs/+` -
Architecture overview: `+CLAUDE.md+` - Technology stack:
`+Docs/STACK.md+`
