...
WP Praxis manifests are declarative configuration files that define
symbolic operations, workflows, and dispatch rules. Manifests can be
written in YAML, TOML, or Dhall formats.
...
|`+dispatch+` |string |No |Executor hint: "`rust_injector`",
"`php_engine`", "`auto`"

|`+parameters+` |object |No |Symbol parameters

|`+dependencies+` |array[string] |No |Symbol dependencies (other symbol
names)
...
|`+error-handling+` |string |No |Error handling: "`stop`", "`continue`"
(default: "`stop`")
...
|`+continue-on-error+` |boolean |No |Continue if step fails (default:
false)
...
. *References*: Symbol dependencies exist
. *Cycles*: No circular dependencies
. *Uniqueness*: Symbol and workflow names are unique
. *Constraints*: Valid enum values (e.g., symbol types)
...
For implementation details, see the link:API.md[API Documentation].
